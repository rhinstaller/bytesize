...
class TranslationTestCase(unittest.TestCase):
    ...
    def testTranslated(self):
        ...
        for lang in  self.TEST_LANGS:
            ...
            self.assertEquals(s, Size(size.parseSpec("56.19 MiB")))

            # Check native parsing
            self.assertEquals(s, Size(size.parseSpec("56.19 %s%s" % (_("Mi"), _("B")))))

            # Check native parsing, all lowercase
            self.assertEquals(s, Size(size.parseSpec(("56.19 %s%s" % (_("Mi"), _("B"))).lower())))

            # Check native parsing, all uppercase
            self.assertEquals(s, Size(size.parseSpec(("56.19 %s%s" % (_("Mi"), _("B"))).upper())))

            # If the radix separator is not a period, repeat the tests with the
            # native separator
            radix = locale.nl_langinfo(locale.RADIXCHAR)
            if radix != '.':
                self.assertEquals(s, Size(size.parseSpec("56%s19 MiB" % radix)))
                self.assertEquals(s, Size(size.parseSpec("56%s19 %s%s" % (radix, _("Mi"), _("B")))))
                self.assertEquals(s, Size(size.parseSpec(("56%s19 %s%s" % (radix, _("Mi"), _("B"))).lower())))
                self.assertEquals(s, Size(size.parseSpec(("56%s19 %s%s" % (radix, _("Mi"), _("B"))).upper())))
